...
export const NavBar = (props: { isSignedIn?: boolean; isSignedOut?: boolean; isPremium?: boolean }) => {
  return (
    <div>
      <div className="flex h-15 w-full items-center justify-between px-4 sm:px-6 md:h-25 md:px-8">
        ...
        {props.isSignedIn && (
          <div className="flex h-fit items-center gap-6 xl:mr-21">
            ...
            {!props.isPremium && (
              <button className="flex cursor-pointer items-center gap-3 rounded-xl bg-gradient-to-r from-error-100 to-primary-200 px-3 py-2 text-xs font-semibold text-white">
                <UpgradeIcon />
                <span>Upgrade</span>
              </button>
            )}
            <BellIcon />
            <div>
              <Image src="/General/Profile.png" alt="User Avatar" width={40} height={40} className="rounded-full" />
            </div>
          </div>
        )}
      </div>
